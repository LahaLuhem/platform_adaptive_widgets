...
Cuts a versioned release of `platform_adaptive_widgets`. Bumps the `version:` field
in `pubspec.yaml` via `cider`, finalises the `## Unreleased` block in `CHANGELOG.md`
into a dated section, regenerates `example/pubspec.lock` so its `path: ../` parent
entry tracks the new version, commits all three files, creates a SemVer tag, and
