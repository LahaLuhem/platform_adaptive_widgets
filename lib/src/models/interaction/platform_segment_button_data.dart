// Per-platform records for PlatformSegmentButton (no shared private base —
// the Material and Cupertino visual surfaces don't overlap).
...
///
/// Pass this via `PlatformSegmentButton.materialSegmentButtonData` when
/// tuning Material rendering. The fields declared here have no Cupertino
/// equivalent.
///
/// Not generic on the segment value type — none of these fields reference
/// `T`. Pass directly: `MaterialSegmentButtonData(direction: .vertical)`,
...
final class MaterialSegmentButtonData {
  ...
  /// Insets applied when the segmented button is laid out in an expanded
  /// (full-width) configuration.
  final EdgeInsets? expandedInsets;

  /// Whether tapping the currently-selected segment is allowed to clear the
  /// selection. Functional, Material-only — Cupertino has no equivalent
  /// "empty selection" affordance (use
  /// [CupertinoSegmentButtonData.isMomentary] for tap-resets-after).
  /// Defaults to [kDefaultSegmentButtonEmptySelectionAllowed].
  final bool emptySelectionAllowed;

  /// Whether to show a check-mark glyph on the selected segment.
  /// Functional, Material-only. Defaults to
  /// [kDefaultSegmentButtonShowSelectedIcon].
  final bool showSelectedIcon;

  /// Layout direction. Material-only — Cupertino's sliding segmented
  /// control is always horizontal. Defaults to
  /// [kDefaultSegmentButtonDirection].
...
///
/// Pass this via `PlatformSegmentButton.cupertinoSegmentButtonData` when
/// tuning Cupertino rendering. The fields declared here have no Material
/// equivalent.
///
/// Generic on the segment value type because [disabledChildren] is a
/// `Set<T>`. Type inference handles most call sites
/// (`CupertinoSegmentButtonData(thumbColor: …)` infers `T` from the
/// surrounding `PlatformSegmentButton<T>`'s slot).
final class CupertinoSegmentButtonData<T extends Object> {
  /// Subset of segment values that should be disabled (un-tappable).
  /// Functional, Cupertino-only — Material's [SegmentedButton] has no
  /// per-segment-disabled list (per-segment `enabled` lives on
  /// [ButtonSegment], which the package doesn't currently surface).
  /// Defaults to [kDefaultCupertinoSegmentButtonDisabledChildren].
  ...
  /// Cupertino-only. Defaults to
  /// [kDefaultCupertinoSegmentButtonProportionalWidth].
  final bool proportionalWidth;

  /// Whether the selection resets after each tap (momentary behaviour).
  /// Functional, Cupertino-only. Defaults to
  /// [kDefaultCupertinoSegmentButtonIsMomentary].
