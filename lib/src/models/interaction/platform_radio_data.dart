...
///
/// Inherited by [MaterialRadioData] and [CupertinoRadioData] so each per-platform record carries the
/// shared-visual surface via `super.x` constructor forwarding. Library-private, never exported from
/// the package.
///
/// Not generic on the radio's value type: the shared-visual fields are all independent of `T`, so
/// the data classes stay un-parameterised even though [PlatformRadio] itself is `PlatformRadio<T>`.
///
...
abstract class const _PlatformRadioData({
  ...
  ///
  /// Maps to [Radio.fillColor] on Android directly (the underlying widget already accepts a
  /// [WidgetStateProperty]) and to [CupertinoRadio.fillColor] on iOS after resolving to a single
  /// [Color]. See `APPENDIX.md#cross-platform-field-mappings` for the resolution policy.
  final WidgetStateProperty<Color?>? fillColor,
});

/// Material-only configuration for [PlatformRadio].
///
/// Pass this via `PlatformRadio.materialRadioData` when tuning Material rendering. Inherited
...
///
/// Not generic on the radio's value type. These are all visual / interaction fields independent of
/// `T`. Pass directly: `MaterialRadioData(visualDensity: .compact)`, without spelling the type
/// parameter.
...
///
/// Pass this via `PlatformRadio.cupertinoRadioData` when tuning Cupertino rendering. Inherited
...
///
/// Not generic on the radio's value type. These are all visual fields independent of `T`.
final class const CupertinoRadioData({
  ...
  /// Colour applied when the radio is **not** selected. Cupertino-only, Material's `Radio` derives
  /// the inactive look from theme.
  final Color? inactiveColor,

  /// Whether to render a checkmark glyph instead of a filled inner circle when selected. Cupertino-only.
  /// Defaults to [kDefaultCupertinoRadioUseCheckmarkStyle].
