...
///
/// `CupertinoSlider.thumbColor` is non-nullable with an inline default of `CupertinoColors.white`.
/// No public `CupertinoSlider.defaultThumbColor` constant exists upstream, so the package owns this
/// constant. The Cupertino branch substitutes it when the widget's flat `PlatformSlider.thumbColor`
/// (typed `Color?`) is `null`.
...
///
/// Inherited by [MaterialSliderData] and [CupertinoSliderData] so each per-platform record carries
...
///
/// Pass this via `PlatformSlider.materialSliderData` when tuning Material rendering. Inherited
...
///
/// Houses both Material-only visual fields (`inactiveColor`, `secondaryActiveColor`, `overlayColor`,
/// `mouseCursor`, `label`, `padding`) and Material-only functional fields (`secondaryTrackValue`,
/// `semanticFormatterCallback`, `focusNode`, `autofocus`, `allowedInteraction`) per the platform-only
/// bucket in `APPENDIX.md#field-classification`, Cupertino's slider exposes none of these.
final class const MaterialSliderData({
  super.activeColor,
  super.thumbColor,

  /// Value for the secondary track (e.g. buffered progress). Functional, but Material-only. Cupertino's
  /// slider has no secondary track.
  ...
  ///
  /// Functional, but Material-only. Cupertino's slider auto-generates accessibility semantics.
  final SemanticFormatterCallback? semanticFormatterCallback,

  /// Focus node for the slider. Functional, Material-only. Cupertino's slider does not participate
  /// in focus traversal.
  final FocusNode? focusNode,

  /// Whether the slider should autofocus. Defaults to [kDefaultSliderAutofocus].
  final bool autofocus = kDefaultSliderAutofocus,

  /// Allowed interaction type (slide-only, tap-only, both). Functional, but Material-only.
  final SliderInteraction? allowedInteraction,

  /// Padding around the slider.
  final EdgeInsetsGeometry? padding,

  /// Controls when the value-indicator label (above the thumb) is visible. Material-only. Cupertino's
  /// slider has no value-indicator. When `null`, Material applies its theme-driven default.
...
///
/// Pass this via `PlatformSlider.cupertinoSliderData` when tuning Cupertino rendering. [CupertinoSlider]
/// has no Cupertino-only fields beyond the shared-visual surface, so this record exists only to let
/// callers override the inherited [activeColor] / [thumbColor] on the Cupertino branch without
/// affecting Android. Future Cupertino-only fields would slot in here.
